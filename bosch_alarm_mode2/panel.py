...
class Area(PanelEntity):
    ...
    def is_triggered(self) -> bool:
        return bool((self.is_armed() or self.is_pending()) and self._alarms.intersection(
...
class Panel:
    ...
    def __init__(self, host: str, port: int, automation_code: str | None, installer_or_user_code: str | None) -> None:
        ...
        self._history_cmd: int | None = None
    ...
    def connection_status(self) -> bool:
        return bool(self._connection is not None and self.points and self.areas)
    ...
    async def _load_history(self) -> None:
        if not self._connection or not self._history_cmd:
            return
    ...
    async def _authenticate_remote_user(self) -> None:
        ...
        try:
            ...
            await self._connection.send_command(CMD.LOGIN_REMOTE_USER, creds.to_bytes(4, "big"))
        except Exception:
            raise PermissionError(
                "Authentication failed, please check your passcode.")

    async def _authenticate_automation_user(self, user_type: int) -> None:
        if not self._connection or not self._automation_code:
            return
    ...
    async def _load_names_cf01(self, name_cmd: int, enabled_ids: list[int], id_size: int = 2) -> dict[int, str]:
        if not self._connection:
            return {}
        names: dict[int, str] = {}
    ...
    async def _get_alarms_for_priority(self, priority: int, last_area: int | None = None, last_point: int | None = None) -> None:
        if not self._connection:
            return
    ...
    async def _load_entity_status(self, status_cmd: int, entities: dict[int, Any], id_size: int = 2) -> None:
        if not self._connection:
            return
        if not entities:
            return
