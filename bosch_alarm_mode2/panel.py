...
class Area(PanelEntity):
    ...
    def is_triggered(self) -> bool:
        return bool(
            (self.is_armed() or self.is_pending())
            and self._alarms.intersection(ALARM_MEMORY_PRIORITY_ALARMS)
        )
...
class Panel:
    ...
    def _area_ready_consumer(self, data: bytearray) -> int:
        ...
        if area_id in self.areas:
            ...
            LOG.debug("Area %d: %s (%d faults)" % (area_id, AREA_READY_STATUS.TEXT[ready_status], faults))
        return 5
